...
uint32* AudioFilterPtr       = (uint32 *)AUDIO_FILTER_0_BASE;
volatile uint32 switch_value = 0;
...
{
    switch_value = *(SwitchPtr + 3);
    *(SwitchPtr + 3) = switch_value;
    
    // switch 0 value is high (0x01), audio is filtered with the low pass filter
    if(switch_value == 1)
    {
        *(AudioFilterPtr + 1) = 0;
    }
