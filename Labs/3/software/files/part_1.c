...
{
	bool pbPushed = true; // create a bool to store the state of the pushbuttons, initialize to false
	if(pushbuttons == 15) // (0b1111) check to see if KEY1 is pushed
	{
		pbPushed = false; // if it is pushed, set to true
	}
	
	else
	{
		while(pbPushed) // infinitely loop while KEY1 is pushed
		{
    ...
    *Hex_Pointer = hexNumbers[count]; // display the array on the hex display
	...
	while(1)
	{
		...
		*Hex_Pointer = hexNumbers[count]; // display the array on the hex display
		...
		CPB = true; // reset to false for the next iteration
	}
	return 0;
}
